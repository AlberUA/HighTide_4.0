(() => {
    ...
    (() => {
        ...
        window.onload = function() {
            ...
            const inputSearch = document.querySelector(".search-featured__input");
            if (inputSearch) inputSearch.oninput = function() {
                let val = this.value.trim().toLowerCase();
                let blogItems = document.querySelectorAll(".blog-item-card__title-search");
                if (val != "") blogItems.forEach((function(elem) {
                    if (elem.innerText.toLowerCase().search(val) == -1) elem.closest(".categories-featured__item").classList.add("hide"); else elem.closest(".categories-featured__item").classList.remove("hide");
                })); else blogItems.forEach((function(elem) {
                    elem.closest(".categories-featured__item").classList.remove("hide");
                }));
            };
            const inputSearchFaq = document.querySelector(".search-faq__input");
            if (inputSearchFaq) inputSearchFaq.oninput = function() {
                let val = this.value.trim().toLowerCase();
                let faqItems = document.querySelectorAll(".item-questions-faq__title");
                if (val != "") faqItems.forEach((function(elem) {
                    if (elem.innerText.toLowerCase().search(val) == -1) elem.closest(".questions-faq__item").classList.add("hide"); else elem.closest(".questions-faq__item").classList.remove("hide");
                })); else faqItems.forEach((function(elem) {
                    elem.closest(".questions-faq__item").classList.remove("hide");
                }));
            };
        };
        window["FLS"] = true;
